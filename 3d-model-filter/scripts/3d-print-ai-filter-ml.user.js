// ==UserScript==
// @name         3D Print AI Filter - ML
// @namespace    https://github.com/3d-print-ai-filter
// @version      3.0.0
// @description  Filter AI models using TensorFlow.js machine learning
// @author       Anonymous
...
// ML version - uses tensorflow.js for smarter detection
// you can train your own model - see the training folder
...
(function() {
    ...
    var cfg = {
        ...
        analyzeImages: true,
        useMLModel: true,
        mlModelUrl: '', // set this to your hosted model.json
        threshold: 65,
        mlThreshold: 0.6, // 0-1 for ML predictions
        minLikes: 0,
        minDownloads: 0,
        minMakes: 0,
        whitelist: [],
    ...
    function saveCfg() {
        try { GM_setValue('aif_ml_cfg', JSON.stringify(cfg)); } catch(e) {}
    }
    ...
    async function loadModel() {
        if (!cfg.mlModelUrl || !cfg.useMLModel) return;
        if (mlModel || mlLoading) return;

        mlLoading = true;
        updateMLStatus('loading', 'Loading model...');

        try {
            mlModel = await tf.loadLayersModel(cfg.mlModelUrl);
            mlError = null;
            updateMLStatus('ok', 'Model loaded');
            console.log('aif: ml model loaded');
        } catch(e) {
            mlError = e.message;
            mlModel = null;
            updateMLStatus('err', 'Load failed: ' + e.message);
            console.log('aif: ml load error', e);
        }

        mlLoading = false;
    }

    async function predictML(img) {
        if (!mlModel || !img) return null;
        ...
        try {
            ...
            var pred = await mlModel.predict(tensor).data();
            tensor.dispose();

            // assuming binary classification: [human_prob, ai_prob]
            // or single output: ai_prob
            var aiProb = pred.length > 1 ? pred[1] : pred[0];
            return aiProb;
        } catch(e) {
            console.log('aif: ml predict error', e);
            return null;
        }
    }

    function updateMLStatus(type, msg) {
        var el = document.getElementById('aif-ml-status');
        if (!el) return;
        el.className = 'ml-status ' + type;
        el.textContent = msg;
    }

    // ===== DETECTION (same as advanced + ML) =====
        ...
        for (var i = 0; i < aiTools.length; i++) {
            ...
            if (idx !== -1) {
                ...
                if (ctx.includes('not ') || ctx.includes('no ') || ctx.includes('without') ||
                    ctx.includes('hate') || ctx.includes('ban') || ctx.includes('filter')) continue;

                if (ctx.includes('made with') || ctx.includes('created with') || ctx.includes('generated')) {
                    score += 35;
    ...
    function checkQuality(card) {
        ...
        if (!txt.includes('print') && !txt.includes('pla') && !txt.includes('petg')) {
            score += 20;
            reasons.push('no print info');
        }
        if (txt.length < 30) {
            score += 25;
            reasons.push('tiny desc');
    ...
    function analyzeImg(img) {
        return new Promise(function(resolve) {
            ...
            try {
                ...
                var smooth = 1 - (diff / n / 765);
                if (smooth > 0.85) { score += 25; reasons.push('very smooth'); }
                else if (smooth > 0.72) { score += 12; reasons.push('smooth'); }
    ...
    function getCreator(card) {
        var sels = ['[class*="author"]','[class*="creator"]','[class*="user"]','.username','a[href*="/user/"]'];
    ...
    async function processCard(card, site) {
        ...
        if (creator && cfg.whitelist.includes(creator)) { stats.clean++; return; }
        if (modelId && cfg.markedOk.includes(modelId)) { stats.clean++; return; }
        ...
        if (!dominated && cfg.filterSuspected) {
            ...
            // try ML first
            if (cfg.useMLModel && mlModel && img) {
                var mlProb = await predictML(img);
                if (mlProb !== null) {
                    if (mlProb >= cfg.mlThreshold) {
                        h.score += 50;
                        allReasons.push('ML:' + (mlProb * 100).toFixed(0) + '%');
                    } else {
                        h.score -= 15; // ML says probably not AI
                    }
                }
            }

            // fallback to image heuristics
            if (cfg.analyzeImages && img && !mlModel) {
    ...
    function flagCard(card, type, reasons, creator, modelId) {
        ...
        if (modelId) {
            ...
            okBtn.onclick = function(e) {
                ...
                if (!cfg.markedOk.includes(modelId)) { cfg.markedOk.push(modelId); saveCfg(); }
                unflag(card);
            };
        ...
        if (creator) {
            ...
            wlBtn.onclick = function(e) {
                e.preventDefault(); e.stopPropagation();
                if (!cfg.whitelist.includes(creator)) { cfg.whitelist.push(creator); saveCfg(); }
                unflag(card);
            };
            ...
            blBtn.onclick = function(e) {
                e.preventDefault(); e.stopPropagation();
                if (!cfg.blacklist.includes(creator)) { cfg.blacklist.push(creator); saveCfg(); }
            };
    ...
    function createPanel() {
        ...
        p.id = 'aif-panel';
        p.innerHTML = '<h3>AI Filter (ML)</h3>' +
            '<div id="aif-ml-status" class="ml-status">No model</div>' +
            '<div class="stats">' +
            ...
            '<label><input type="checkbox" id="aif-useml"> Use ML model</label>' +
            '<input type="text" id="aif-mlurl" placeholder="Model URL (model.json)">' +
            '<button id="aif-loadml" style="margin-top:4px">Load Model</button>' +
            '</div>' +
            '<div class="sec">' +
            '<label>Heuristic threshold: <span id="aif-tv">65</span></label>' +
            '<input type="range" id="aif-thresh" min="0" max="100" value="65">' +
            '<label>ML threshold: <span id="aif-mtv">60</span>%</label>' +
            '<input type="range" id="aif-mlthresh" min="0" max="100" value="60">' +
            '</div>' +
            '<div class="sec">' +
    ...
    function bindEvents() {
        ...
        $('aif-eng').onchange = function() { cfg.filterEngagement = this.checked; saveCfg(); rescan(); };
        $('aif-hl').onchange = function() { cfg.highlightOnly = this.checked; saveCfg(); rescan(); };

        $('aif-useml').onchange = function() { cfg.useMLModel = this.checked; saveCfg(); };
        $('aif-mlurl').onchange = function() { cfg.mlModelUrl = this.value.trim(); saveCfg(); };
        $('aif-loadml').onclick = function() { loadModel(); };
        ...
        $('aif-mlthresh').oninput = function() {
            cfg.mlThreshold = parseInt(this.value) / 100;
            $('aif-mtv').textContent = this.value;
            saveCfg();
        };
        $('aif-mlthresh').onchange = rescan;
    ...
    function syncPanel() {
        ...
        $('aif-useml').checked = cfg.useMLModel;
        $('aif-mlurl').value = cfg.mlModelUrl;
        $('aif-thresh').value = cfg.threshold;
        $('aif-tv').textContent = cfg.threshold;
        $('aif-mlthresh').value = Math.round(cfg.mlThreshold * 100);
        $('aif-mtv').textContent = Math.round(cfg.mlThreshold * 100);
    }
    ...
    function showManage() {
        ...
        m.innerHTML = '<h3>Manage Lists</h3>' +
            '<div><h4>Trusted</h4><textarea id="aif-wl">' + cfg.whitelist.join('\n') + '</textarea></div>' +
            '<div><h4>Blocked</h4><textarea id="aif-bl">' + cfg.blacklist.join('\n') + '</textarea></div>' +
    ...
    function doExport() {
        var data = { whitelist: cfg.whitelist, blacklist: cfg.blacklist, markedOk: cfg.markedOk };
        ...
        inp.onchange = function(e) {
            ...
            r.onload = function(ev) {
                try {
                    ...
                    if (d.whitelist) cfg.whitelist = [...new Set(cfg.whitelist.concat(d.whitelist))];
                    if (d.blacklist) cfg.blacklist = [...new Set(cfg.blacklist.concat(d.blacklist))];
                    if (d.markedOk) cfg.markedOk = [...new Set(cfg.markedOk.concat(d.markedOk))];
                    saveCfg();
                    rescan();
                    alert('Imported!');
    ...
    function init() {
        ...
        // try to load model if url is set
        if (cfg.mlModelUrl && cfg.useMLModel) {
            loadModel();
        }
